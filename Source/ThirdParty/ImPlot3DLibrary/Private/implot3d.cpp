...
void ComputeActiveFaces(bool* active_faces, const ImPlot3DQuat& rotation, const ImPlot3DAxis* axes, int* plane_2d = nullptr) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (fabs(rot_face_n[i].z) < 0.025) {
            ...
            bool is_inverted = ImPlot3D::ImHasFlag(axes[i].Flags, ImPlot3DAxisFlags_Invert);
...
void ShowAxisContextMenu(ImPlot3DAxis& axis) {
    ...
    if (ImGui::Checkbox("Invert", &inverted))
        ImPlot3D::ImFlipFlag(axis.Flags, ImPlot3DAxisFlags_Invert);

    ImGui::Separator();

    ImGui::BeginDisabled(axis.Label.empty());
    if (ImGui::Checkbox("Label", &label))
        ImPlot3D::ImFlipFlag(axis.Flags, ImPlot3DAxisFlags_NoLabel);
    ImGui::EndDisabled();

    if (ImGui::Checkbox("Grid Lines", &grid))
        ImPlot3D::ImFlipFlag(axis.Flags, ImPlot3DAxisFlags_NoGridLines);
    if (ImGui::Checkbox("Tick Marks", &ticks))
        ImPlot3D::ImFlipFlag(axis.Flags, ImPlot3DAxisFlags_NoTickMarks);
    if (ImGui::Checkbox("Tick Labels", &labels))
        ImPlot3D::ImFlipFlag(axis.Flags, ImPlot3DAxisFlags_NoTickLabels);
}
...
void ShowPlotContextMenu(ImPlot3DPlot& plot) {
    ...
    if ((ImGui::BeginMenu("Legend"))) {
        if (ShowLegendContextMenu(plot.Items.Legend, !ImPlot3D::ImHasFlag(plot.Flags, ImPlot3DFlags_NoLegend)))
            ImPlot3D::ImFlipFlag(plot.Flags, ImPlot3DFlags_NoLegend);
        ImGui::EndMenu();
    }
    ...
    if ((ImGui::BeginMenu("Settings"))) {
        ...
        if (ImGui::MenuItem("Title", nullptr, plot.HasTitle()))
            ImPlot3D::ImFlipFlag(plot.Flags, ImPlot3DFlags_NoTitle);
        ImGui::EndDisabled();
        if (ImGui::MenuItem("Clip", nullptr, !ImPlot3D::ImHasFlag(plot.Flags, ImPlot3DFlags_NoClip)))
            ImPlot3D::ImFlipFlag(plot.Flags, ImPlot3DFlags_NoClip);
        if (ImGui::MenuItem("Mouse Position", nullptr, !ImPlot3D::ImHasFlag(plot.Flags, ImPlot3DFlags_NoMouseText)))
            ImPlot3D::ImFlipFlag(plot.Flags, ImPlot3DFlags_NoMouseText);
        ImGui::EndMenu();
    }
}
...
void ImDrawList3D::SortedMoveToImGuiDrawList() {
    ...
    if (_TextureBuffer.Size > 1) {
        ...
        for (idx_out = idx_out_begin; idx_out < idx_out_end; idx_out += 3) {
            ...
            unsigned int idx_in_1 = (unsigned int)(*idx_out - idx_offset);
            ...
            for (int j = 0; j < _TextureBuffer.Size; j++)
                if (idx_in_1 >= _TextureBuffer[j].VtxIdx)
